...
st.caption("数据源：Yahoo Finance (期货) + MetalPriceAPI (现货) | COMEX 期货 + 伦敦金现货 + 人民币金价 | 适合每日收盘盘点")
...
try:
    ...
    st.caption(f"期货数据同步交易日 (UTC): {latest_date} | 现货数据源: MetalPriceAPI (实时)")
    st.markdown("---")
    ...
    st.info(
        ...
        "- 伦敦金现货由 MetalPriceAPI 提供实时报价。\n"
        "- 上海金参考价通过国际金价 × USD/CNY 汇率 ÷ 31.1035 换算得出。\n"
