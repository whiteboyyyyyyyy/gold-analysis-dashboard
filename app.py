...
def get_common_dates_safe(df1, df2):
    """安全比對：先統一轉成 datetime64[us]，再取交集"""
    s1 = df1.set_index('日期')['收市'].copy()
    s2 = df2.set_index('日期')['收市'].copy()
    # 確保 index 都是 datetime64[us]
    s1.index = pd.to_datetime(s1.index).tz_localize(None)
